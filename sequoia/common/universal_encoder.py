...
resnet_output_space = spaces.Box(
    0.0,
    1.0,
    shape=[
        1000,
...
@singledispatch
def create_encoder(
    input_space: Space, output_space: Space, budget: int = None, **kwargs
) -> Tuple[nn.Module, Space]:
...
class DictEncoder(nn.Module):
    ...
    def forward(self, x: Dict[str, Tensor]):
        return {key: self.encoders[key](tensor) for key, tensor in x.items()}
...
def dict_encoder(
    ...
    **kwargs,
    ...
    assert (
        latent_dims == hidden_dims
    ), "The sum of latent spaces didn't equal the prescribed hidden dims?"
...
@create_encoder.register
def tuple_encoder(
    input_space: spaces.Tuple, output_space: Space, budget: int = None, **kwargs
) -> nn.Module:
    ...
    return create_encoder(
        input_space=spaces.Dict(
            {str(i): subspace for i, subspace in enumerate(input_space.spaces)}
        ),
        output_space=output_space,
        budget=budget,
        **kwargs,
...
def image_encoder(
    input_space: Image, output_space: Space, budget: int = None, **kwargs
) -> nn.Module:
    ...
    return nn.Sequential(encoder, create_encoder(hidden_space, output_space, budget=budget))
...
def discrete_encoder(
    input_space: spaces.Discrete, output_space: Space, budget: int = None, **kwargs
) -> nn.Module:
...
def multidiscrete_encoder(
    input_space: spaces.MultiDiscrete, output_space: Space, budget: int = None
) -> nn.Module:
