...
# Lorenzo Pellegrini for typing stuff
...
class BaseStrategy(_BaseStrategy):
    ...
    def train(
        ...
        **kwargs,
        ...
        elif isinstance(experiences, gym.Env):
            print(f"Being passed a gym Env as an experience: {experiences}")
            # NOTE: Here we just make it into an experience 'stream'?
            experiences = [experiences]

        eval_streams: List[List[Experience]]
        if eval_streams is None:
            eval_streams = [experiences]
        elif isinstance(eval_streams, gym.Env):
            eval_streams = [eval_streams]
        for i, exp in enumerate(eval_streams):
            if isinstance(exp, (Experience, gym.Env)):
                eval_streams[i] = [exp]
    ...
    def eval_epoch(self, **kwargs):
        for self.mb_it, (self.mb_x, self.mb_y, self.mb_task_id) in enumerate(
            self.dataloader
        ):
