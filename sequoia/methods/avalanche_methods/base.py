...
@dataclass
class AvalancheMethod(Method, HyperParameters, target_setting=TaskIncrementalSetting):
    ...
    strategy_class: ClassVar[Type[BaseStrategy]]

    # Class Variable to hold the types of models available.
    ...
    # Class Variable to hold the types of optimziers available.
    available_optimizers: ClassVar[Dict[str, Type[Optimizer]]] = {
        "sgd": SGD,
        "adam": optim.Adam,
        "rmsprop": optim.RMSprop,
    }
    # Class variable to hold the types of loss functions available.
    available_criterions: ClassVar[Dict[str, Type[nn.Module]]] = {
        "cross_entropy_loss": nn.CrossEntropyLoss,
    }

    # The model.
    model: Module = choice(available_models, default=SimpleCNN)
    # The optimizer to use.
    optimizer: Type[Optimizer] = choice(available_optimizers, default=optim.Adam)
    # The loss criterion to use.
    criterion: Type[Module] = choice(
    ...
    def configure(self, setting: ClassIncrementalSetting) -> None:
        ...
        self.cl_strategy = self.create_cl_strategy(setting)

    def create_cl_strategy(self, setting: ClassIncrementalSetting):
    ...
    def make_optimizer(self, **kwargs) -> Optimizer:
        ...
        return optimizer_class(self.model.parameters(), **options)

    def fit(self, train_env: PassiveEnvironment, valid_env: PassiveEnvironment):
        # Reset the optimizer, in case some new weights were added?
        self.make_optimizer()
    ...
    while not test_env.is_closed() and episode < max_episodes:
        ...
        with tqdm.tqdm(desc="Eval epoch") as pbar:
            while not done:
                ...
                strategy.logits = strategy.model(strategy.mb_x)
