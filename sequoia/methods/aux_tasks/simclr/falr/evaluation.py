...
def evaluate_features(
    ...
    epoch: int,
    log_wandb: bool,
    background_queue: Optional[mp.Queue] = None,
) -> None:
    encoder.eval()
    projector.eval()
    classifier.eval()
    if hp.experiment not in {ExperimentType.CONTRASTIVE, ExperimentType.CLASS_CONTRASTIVE}:
        evaluate_classifier(
            hp, encoder, projector, classifier, train_loader, epoch, log_wandb, "train"
        )
        evaluate_classifier(
            hp, encoder, projector, classifier, test_loader, epoch, log_wandb, "test"
        )
