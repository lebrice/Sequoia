...
class EWCTask(AuxiliaryTask):
    ...
    def consolidate(self, fisher):
        for n, p in self._shared_net.named_parameters():
            n = n.replace('.', '__')
            self._model.register_buffer(f'{n}_mean', p.data.clone())
            self._model.register_buffer(f'{n}_fisher', fisher[n].data.clone())
    ...
    def ewc_loss(self):
        try:
            ...
            for n, p in self._shared_net.named_parameters():
                ...
                n = n.replace('.', '__')
                mean = getattr(self._model, f'{n}_mean')
                fisher = getattr(self._model, f'{n}_fisher')
