...
@dataclass
class TaskIncremental_EWC(Experiment):
    ...
    def run(self):
        ...
        for i in range(self.state.i, self.n_tasks):
            ...
            # set dataloader with which Fisher will be calculated
            if self.config.debug:
                self.model.current_task_loader = self.get_dataloader(Subset(train_i, range(100)))
            else:
                self.model.current_task_loader = self.get_dataloader(train_i)
