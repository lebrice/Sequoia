        ...
        for root, _, fnames in sorted(os.walk(d)):
            for fname in sorted(fnames):
                if is_image_file(fname):
                    ...
                    item = (path, int(class_to_idx[target]))
                    images.append(item)
...
class ImageFolder(VisionDataset):
    ...
    def __init__(self, root, split='train', transform=None, target_transform=None,
        ...
        root = f'{str(root)}/{split}/'   
        super(ImageFolder, self).__init__(root, transform=transform,
                                            target_transform=target_transform)
        self.classes_idx = classes_idx
        classes, class_to_idx = find_classes(self.root, self.classes_idx)
        ...
        self.loader = loader
        self.targets = torch.Tensor([ torch.tensor(img[1]) for img in self.imgs])
