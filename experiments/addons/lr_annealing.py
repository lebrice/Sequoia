...
class LRannealer(ExperimentAddon):
    ...
    def adjust_learning_rate(self, optimizer, epoch, step_in_epoch, total_steps_in_epoch, epochs):
        ...
        lr = self.linear_rampup(epoch, self.config.lr_rampup_epochs) * (current_lr - initial_lr) + initial_lr
    ...
    def step(self, global_step:int, epoch: int=None, epoch_length: int=None, update_number: int=None, **kwargs):
        if self.config.lr_rampdown_epochs or self.config.lr_rampup_epochs:     
            assert epoch!=None
