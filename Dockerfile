...
RUN pip install \ 
    fasteners \ 
    # Needed to build atari_py:
    lockfile \
    pybullet wandb tqdm tensorflow bs4 \
...
RUN mkdir /workspace/tools && cd /workspace/tools
RUN git clone https://github.com/openai/mujoco-py.git && cd mujoco-py
...
# RUN cd /workspace/tools && git clone https://github.com/fgolemo/pytorch-a2c-ppo-acktr-gail.git && cd pytorch-a2c-ppo-acktr-gail && pip install -e .
RUN mkdir /workspace/dev
COPY .netrc /home/toolkit/.netrc
RUN ln -s /mnt/home/dev ~/dev
